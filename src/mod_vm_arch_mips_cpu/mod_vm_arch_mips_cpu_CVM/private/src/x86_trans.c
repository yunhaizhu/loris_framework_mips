...
void mips64_set_ra(mips64_jit_tcb_t *b, m_va_t ret_pc)
{
 ...
 * main loop.
 */
static void mips64_try_direct_far_jump(cvm_arch_mips_cpu_t *cpu, mips64_jit_tcb_t *b,
                                       m_va_t new_pc)
{
    std_u64_t new_page;
...
/* Set Jump */
static void mips64_set_jump(cvm_arch_mips_cpu_t *cpu, mips64_jit_tcb_t *b,
...
typedef void (*memop_fast_access)(mips64_jit_tcb_t *b, std_int_t target);

/* Fast LW */
STD_CALL static void mips64_memop_fast_lw(mips64_jit_tcb_t *b, std_int_t target)
{
...
/* Fast SW */
STD_CALL static void mips64_memop_fast_sw(mips64_jit_tcb_t *b, std_int_t target)
{
...
static void mips64_emit_memop_fast32(mips64_jit_tcb_t *b, std_int_t write_op,
                                     ...
                                     memop_fast_access op_handler)
{
    ...
    if (write_op) {
        amd64_mov_reg_membase(b->jit_ptr,AMD64_RBX,
                              AMD64_RCX, OFFSET(mts32_entry_t,dirty_bit),1);
        amd64_test_reg_reg(b->jit_ptr,AMD64_RBX,AMD64_RBX);
    ...
    amd64_test_membase_imm_size(b->jit_ptr,
                                AMD64_RCX, OFFSET(mts32_entry_t, mapped),
                                TRUE, 1);
    u_char *test3 = b->jit_ptr;
    amd64_branch8(b->jit_ptr, X86_CC_NZ, 0, 1);

    amd64_mov_reg_membase(b->jit_ptr, AMD64_RBX,
                          AMD64_RCX, OFFSET(mts32_entry_t, g_bit), 1);
    amd64_test_reg_reg(b->jit_ptr, AMD64_RBX, AMD64_RBX);

    u_char *test4 = b->jit_ptr;
    amd64_branch8(b->jit_ptr, X86_CC_NZ, 0, 1);

    amd64_mov_reg_membase(b->jit_ptr, AMD64_RBX,
                          AMD64_R15,
                          OFFSET(cvm_arch_mips_cpu_t, p_cp0_reg), 8);

    amd64_mov_reg_membase(b->jit_ptr, X86_EAX,
                          AMD64_RBX,
                          sizeof(std_u32_t) * MIPS_CP0_TLB_HI, 4);

    amd64_alu_reg_imm(b->jit_ptr, X86_AND, X86_EAX, MIPS_TLB_ASID_MASK);

    amd64_alu_reg_membase_size(b->jit_ptr, X86_CMP, X86_EAX, AMD64_RCX,
                               OFFSET(mts32_entry_t, asid), 4);

    u_char *test5 = b->jit_ptr;
    amd64_branch8(b->jit_ptr, X86_CC_NZ, 0, 1);
...
STD_CALL static void mips64_emit_memop_fast(cvm_arch_mips_cpu_t *cpu, mips64_jit_tcb_t *b,
...
static void mips64_emit_memop(mips64_jit_tcb_t *b, std_int_t op, std_int_t base, std_int_t offset,
                              std_int_t target, std_int_t keep_ll_bit)
{
...
static asmlinkage void mips64_unknown_opcode(cvm_arch_mips_cpu_t *cpu, std_u32_t opcode)
{
...
static fastcall void mips64_invalid_delay_slot(cvm_arch_mips_cpu_t *cpu)
{
...
/* Trigger IRQs */
void fastcall mips64_trigger_irq()
{
...
void fastcall mips64_exec_break(IN cvm_arch_mips_cpu_t *cpu, IN std_uint_t code)
{
    //    mips64_dump_regs(cpu);
...
void fastcall mips64_exec_syscall(cvm_arch_mips_cpu_t *cpu);
/**
 * mips_emit_syscall
 * @brief
 * @param   cpu
...
void fastcall mips64_trigger_trap_exception(cvm_arch_mips_cpu_t *cpu);
/**
 * mips_emit_teq
 * @brief
 * @param   cpu
 ...
 * @return  int
 */
int mips_emit_tlbr(IN cvm_arch_mips_cpu_t *cpu, IN mips64_jit_tcb_t *b, IN std_uint_t insn)
{
    mips64_set_pc(b, b->start_pc + ((b->mips_trans_pos - 1) << 2));
    amd64_mov_reg_imm(b->jit_ptr, AMD64_RSI, insn);
    amd64_mov_reg_reg(b->jit_ptr, AMD64_RDI, AMD64_R15, 8);
 ...
 * @return  int
 */
int mips_emit_tlbwi(IN cvm_arch_mips_cpu_t *cpu, IN mips64_jit_tcb_t *b, IN std_uint_t insn)
{
    mips64_set_pc(b, b->start_pc + ((b->mips_trans_pos - 1) << 2));
    amd64_mov_reg_imm(b->jit_ptr, AMD64_RSI, insn);
    amd64_mov_reg_reg(b->jit_ptr, AMD64_RDI, AMD64_R15, 8);
