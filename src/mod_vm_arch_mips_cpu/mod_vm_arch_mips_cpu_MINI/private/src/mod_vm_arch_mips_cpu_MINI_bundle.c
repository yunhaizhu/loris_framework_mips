...
/**
 * @file    mod_vm_arch_mips_cpu_MINI_bundle.c
...
static mod_iid_t mod_vm_arch_mips_cpu_iid = MOD_VM_ARCH_MIPS_CPU_IID;
...
{
	STD_ASSERT_RV(mod_register_instance_func(&mod_vm_arch_mips_cpu_iid, mod_vm_arch_mips_cpu_MINI_create_instance)
		      == STD_RV_SUC, STD_RV_ERR_FAIL);
	STD_ASSERT_RV(mod_bundle_IID_register(bundle_service_id, &mod_vm_arch_mips_cpu_iid) == STD_RV_SUC,
		      STD_RV_ERR_FAIL);
...
{
	STD_ASSERT_RV(mod_unregister_instance_func(&mod_vm_arch_mips_cpu_iid) == STD_RV_SUC,);
