 ...
 */
STD_CALL std_rv_t mod_vm_memory_MINI_init(IN mod_vm_memory_t * p_m, IN const std_char_t * arg, IN std_int_t arg_len)
{
	...
	mod_shell_register(p_imp_m->p_mod_shell, key, std_safe_strlen(key, sizeof(key)),
			   shell_stub_mod_vm_memory_MINI_initiate, p_imp_m);

	snprintf(key, sizeof(key), "%lul-%s", p_imp_m->unique_id, "mod_vm_memory_lookup");
	mod_shell_register(p_imp_m->p_mod_shell, key, std_safe_strlen(key, sizeof(key)),
			   shell_stub_mod_vm_memory_MINI_lookup, p_imp_m);

	snprintf(key, sizeof(key), "%lul-%s", p_imp_m->unique_id, "mod_vm_memory_access");
	mod_shell_register(p_imp_m->p_mod_shell, key, std_safe_strlen(key, sizeof(key)),
			   shell_stub_mod_vm_memory_MINI_access, p_imp_m);

	snprintf(key, sizeof(key), "%lul-%s", p_imp_m->unique_id, "mod_vm_memory_map");
	mod_shell_register(p_imp_m->p_mod_shell, key, std_safe_strlen(key, sizeof(key)),
			   shell_stub_mod_vm_memory_MINI_map, p_imp_m);

	snprintf(key, sizeof(key), "%lul-%s", p_imp_m->unique_id, "mod_vm_memory_unmap");
	mod_shell_register(p_imp_m->p_mod_shell, key, std_safe_strlen(key, sizeof(key)),
			   shell_stub_mod_vm_memory_MINI_unmap, p_imp_m);

	return STD_RV_SUC;
}

/**
 * mod_vm_memory_MINI_cleanup
 * @brief   
 * @param   p_m
 * @return  STD_CALL std_rv_t
 */
STD_CALL std_rv_t mod_vm_memory_MINI_cleanup(mod_vm_memory_t * p_m)
{
	mod_vm_memory_imp_t *p_imp_m = (mod_vm_memory_imp_t *) p_m;
	mod_iid_t mod_shell_iid = MOD_SHELL_IID;
	std_char_t key[BUF_SIZE_128] = "\0";
...
/**
 * mod_vm_memory_MINI_initiate
 * @brief   
 * @param   p_m
 * @return  STD_CALL             std_int_t
 */
STD_CALL std_int_t mod_vm_memory_MINI_initiate(IN mod_vm_memory_t * p_m)
{
...
/**
 * mod_vm_memory_MINI_lookup
 * @brief   
 * @param   p_m
 * @param   vaddr
 * @return  STD_CALL             std_void_t  *
 */
STD_CALL std_void_t *mod_vm_memory_MINI_lookup(IN mod_vm_memory_t * p_m, IN std_uint_t vaddr)
{
    return cvm_memory_32_lookup(p_m, vaddr);
}

/**
 * mod_vm_memory_MINI_access
 * @brief   
 * @param   p_m
 * @param   vaddr
 * @param   op_code
 ...
 * @return  STD_CALL             std_void_t  *
 */
STD_CALL std_void_t *mod_vm_memory_MINI_access(IN mod_vm_memory_t * p_m, IN std_uint_t vaddr, IN std_uint_t op_code,
...
/**
 * mod_vm_memory_MINI_map
 * @brief   
 * @param   p_m
 * @param   vaddr
 * @return  STD_CALL             std_void_t
 */
STD_CALL std_void_t mod_vm_memory_MINI_map(IN mod_vm_memory_t * p_m, IN std_uint_t vaddr)
{
	return;
...
/**
 * mod_vm_memory_MINI_unmap
 * @brief   
 * @param   p_m
 * @param   vaddr
 * @return  STD_CALL             std_void_t
 */
STD_CALL std_void_t mod_vm_memory_MINI_unmap(IN mod_vm_memory_t * p_m, IN std_uint_t vaddr)
{
	return;

}

struct mod_vm_memory_ops_st mod_vm_memory_MINI_ops = {
	mod_vm_memory_MINI_init,
	mod_vm_memory_MINI_cleanup,
